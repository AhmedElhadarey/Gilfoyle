...
- Next step: Run `/conductor:run` to start
...
**Next Step**: Run `/conductor:run` to start the Evaluate-Loop
```

## Related

- `/conductor:run` — Run the evaluate-loop on this track
- `/conductor:status` — Check track status
- `/gilfoyle` — Shorthand that creates track + starts implement automatically
