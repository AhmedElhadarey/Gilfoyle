---
name: run
description: "Run the Evaluate-Loop on a track from current state to completion"
arguments:
  - name: track_id
...
# /conductor:run — Automated Evaluate-Loop
...
/conductor:run  # Continue automatically
```
...
- `/gilfoyle` — Single entry point (creates track + runs the evaluate-loop)
